...
describe("Exchange", () => {
  ...
  beforeEach(async () => { // テストごとに実行される。この状態は毎回リフレッシュされる
    [owner, user] = await ethers.getSigners(); // ウォレットを取得

    const Token = await ethers.getContractFactory("Token"); // コントラクトをdeployするためのファクトリーを取得
    token = await Token.deploy("Token", "TKN", toWei(1000000)); // コントラクトをdeployする
    await token.deployed(); // deployするのを待つ

    const Exchange = await ethers.getContractFactory("Exchange"); // コントラクトをdeployするためのファクトリーを取得
    exchange = await Exchange.deploy(token.address); // コントラクトをdeployする
    await exchange.deployed(); // deployするのを待つ
  });
  ...
  describe("tokenToEthSwap", async () => {
    ...
    it("allows zero swaps", async () => {
      ...
      expect(fromWei(userBalance)).to.equal("9999.998602266645584892");
